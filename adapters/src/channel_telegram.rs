...
impl TelegramChannelAdapter {
    ...
    pub fn live(config: TelegramConfig) -> Result<Self, String> {
        ...
        let offset_path = std::env::var("AXIOM_RUNTIME_TOOL_WORKSPACE")
            ...
            .map(|workspace| {
                std::path::PathBuf::from(workspace)
                    .join("telegram_offset_channel.telegram.txt")
            });
        ...
        Ok(Self {
            config,
            transport: TelegramTransport::Live {
                http,
                next_offset,
            },
            offset_path,
            ...
            TelegramTransport::Live { http, .. } => {
                ...
                let body: serde_json::Value = resp.json().map_err(|e| {
                    AdapterError::failed(
                        "telegram.send.parse",
                        classify_reqwest_error(&e),
                        crate::error::RetryClass::NonRetryable,
                    )
                })?;
...
mod tests {
    ...
    fn error_body_preview_passes_short_body() {
        ...
        let preview = if short.len() > MAX_ERROR_BODY_PREVIEW {
            &short[..MAX_ERROR_BODY_PREVIEW]
        } else {
            short
        };
    ...
    fn offline_drain_returns_queued_messages() {
        let mut adapter = TelegramChannelAdapter::new(make_config());
        adapter.send(ChannelMessage::new("telegram", "msg1")).unwrap();
        adapter.send(ChannelMessage::new("telegram", "msg2")).unwrap();
    ...
    fn bot_token_does_not_appear_in_adapter_debug() {
        ...
        // Debug output for TelegramChannelAdapter only shows the mode string,
        // but TelegramConfig derives Debug which does include bot_token.
        // Verify the transport field in debug does NOT expose the token.
        let debug_str = format!("{adapter:?}");
        // The transport field shows "offline" not any URL or token path.
    ...
    fn live_without_offset_path_has_none() {
        ...
        let adapter =
            TelegramChannelAdapter::live_with_offset_path(make_config(), None).unwrap();
        assert!(adapter.offset_path.is_none());
    }
    ...
    fn live_with_explicit_offset_path_sets_field() {
        ...
        let adapter = TelegramChannelAdapter::live_with_offset_path(
            make_config(),
            Some(path.clone()),
        )
        .unwrap();
