use crate::contracts::{AdapterHealth, MemoryAdapter, MemoryEntry, ToolAdapter, ToolCall, ToolOutput};
...
pub struct MemoryToolAdapter {
    ...
    backend: Mutex<Box<dyn MemoryAdapter>>,
}
        ...
        match input.action {
            MemoryToolAction::Store => {
                ...
                let value = input
                    .value
                    .as_deref()
                    .ok_or_else(|| AdapterError::invalid_input("memory.value", "missing value"))?;
                ...
                backend.store(key, value).map_err(|e| {
            ...
            MemoryToolAction::Recall => {
                ...
                if let Some(q) = input.query.as_deref() {
                    let hits = backend.recall(q, input.limit).map_err(|e| {
                        AdapterError::failed("memory.recall", e.to_string(), RetryClass::Retryable)
                    })?;
                    if hits.is_empty() {
                    ...
                    match backend
                        .get(key)
                        .map_err(|e| AdapterError::failed("memory.get", e.to_string(), RetryClass::Retryable))?
                    {
            ...
            MemoryToolAction::Forget => {
                ...
                let removed = backend.delete(key).map_err(|e| {
...
mod tests {
    ...
    fn store_delegates_to_backend_and_recall_finds_entry() {
        ...
        adapter
            ...
            .expect("store ok");
        let out = adapter
            .execute(call("memory.recall", &[("key", "greeting")]))
            .expect("recall ok");
        assert!(out.content.contains("hello world"), "content={}", out.content);
    }
    ...
    fn forget_removes_entry_from_backend() {
        let adapter = MemoryToolAdapter::in_memory(MemoryToolConfig::default());
        adapter
            .execute(call("memory.store", &[("key", "tmp"), ("value", "data")]))
            .expect("store ok");
        let out = adapter
            .execute(call("memory.forget", &[("key", "tmp")]))
            .expect("forget ok");
        assert!(
    ...
    fn recall_by_query_searches_all_entries() {
        ...
        adapter
            ...
            .expect("ok");
        adapter
            .execute(call("memory.store", &[("key", "b"), ("value", "banana")]))
            .expect("ok");
        let out = adapter
            .execute(call("memory.recall", &[("query", "apple")]))
            .expect("ok");
    ...
    fn custom_backend_receives_store_calls() {
        ...
        adapter
            .execute(call("memory.store", &[("key", "x"), ("value", "42")]))
            .expect("ok");
        let out = adapter
            .execute(call("memory.recall", &[("key", "x")]))
            .expect("ok");
