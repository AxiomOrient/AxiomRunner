...
impl IrcChannelAdapter {
    ...
    /// Falls back to offline mode if `config.channel` is None.
    pub fn live(config: IrcConfig) -> Result<Self, String> {
        ...
        let mut stream = stream
            .try_clone()
            .map_err(|e| format!("irc: stream clone failed: {e}"))?;
        ...
        wait_for_welcome(&mut stream)
            .map_err(|e| format!("irc: welcome handshake failed: {e}"))?;
    ...
    fn send(&mut self, message: ChannelMessage) -> AdapterResult<ChannelSendReceipt> {
        ...
        match &mut self.transport {
            ...
            IrcTransport::Live {
                ...
                let body = truncate_to_bytes(&message.body, MAX_IRC_BODY_BYTES);
        ...
        match &mut self.transport {
            ...
            IrcTransport::Live {
                ...
                loop {
                    ...
                    match read_line_with_timeout(stream, &mut line_buf) {
                        Ok(0) => break,
                        Ok(_) => {
                            let full = if read_buf.is_empty() {
                                line_buf
                            } else {
                                let mut s = std::mem::take(read_buf);
                                s.push_str(&line_buf);
                                s
                            };
                            match parse_irc_line(&full) {
                                Some(IrcParsed::Ping(server)) => {
                                    // Best-effort PONG; ignore write errors.
                                    let _ = write!(stream, "PONG :{}\r\n", server);
                                }
                                Some(IrcParsed::Privmsg(text)) => {
                                    results.push(ChannelMessage::new(channel.clone(), text));
                                }
                                None => {}
                            }
                        }
...
mod tests {
    ...
    fn offline_drain_returns_queued_messages() {
        let mut adapter = IrcChannelAdapter::new(make_config());
        adapter.send(ChannelMessage::new("irc", "msg1")).unwrap();
        adapter.send(ChannelMessage::new("irc", "msg2")).unwrap();
    ...
    #[test]
    fn live_without_channel_falls_back_to_offline() {
        let config = IrcConfig::new("irc.example.com:6667", None, "axiom-bot", vec![]).unwrap();
        let adapter = IrcChannelAdapter::live(config).unwrap();
        assert!(!adapter.is_live());
    }
