...
mod tests {
    ...
    fn offline_drain_returns_queued_messages() {
        let mut adapter = SlackChannelAdapter::new(make_config());
        adapter.send(ChannelMessage::new("slack", "msg1")).unwrap();
        adapter.send(ChannelMessage::new("slack", "msg2")).unwrap();
    ...
    fn webhook_url_must_start_with_https() {
        let config = SlackConfig::new("test-bot-token", None, vec![]).unwrap();
        let result =
            config.with_webhook(Some("http://hooks.slack.com/services/T/B/X".to_string()));
