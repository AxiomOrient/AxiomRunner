...
pub fn rank_hybrid_recall(
    ...
    for entry in entries {
        ...
        let semantic_score = semantic_score(query, &query_terms, &entry);

        if !query_is_empty && keyword_score == 0 && semantic_score == 0 {
            continue;
        }
        ...
        let total_score = u64::from(config.weights.keyword) * u64::from(keyword_score)
            + u64::from(config.weights.semantic) * u64::from(semantic_score)
...
pub fn run_sqlite_retention_job(
    memory: &mut SqliteMemoryAdapter,
...
fn semantic_score(query: &str, query_terms: &[String], entry: &MemoryEntry) -> u32 {
...
mod tests {
    ...
    use super::{
        HybridRecallConfig, HybridRecallWeights, compare_hits, keyword_score, recency_score,
        scale_ratio, semantic_score,
    };
    ...
    #[test]
    fn keyword_and_semantic_scoring_work_as_expected() {
        ...
        assert!(semantic_score("policy eng", &query_terms, &target) > 0);
    }
    ...
    fn compare_hits_orders_by_total_then_recency() {
        ...
        let a = super::HybridRecallHit {
            ...
            total_score: u64::from(config.weights.keyword) * 200
                + u64::from(config.weights.semantic) * 200
