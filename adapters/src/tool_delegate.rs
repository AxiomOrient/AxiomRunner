...
use crate::contracts::{
    AdapterHealth, AgentAdapter, AgentRequest, ToolAdapter, ToolCall, ToolOutput,
};
...
mod tests {
    ...
    fn delegate_rejects_when_max_depth_reached() {
        ...
        let err = adapter
            .execute(call_with_task("do something"))
            .expect_err("should fail at max depth");
        assert!(matches!(
            err,
            AdapterError::InvalidInput { field: "delegate", .. }
        ));
    }
    ...
    fn delegate_rejects_missing_task_arg() {
        ...
        let adapter = DelegateToolAdapter::with_agent(Box::new(NeverCalledAgent), 3, 0);
        let err = adapter
            .execute(ToolCall::new("delegate", BTreeMap::new()))
    ...
    fn delegate_at_exact_max_depth_fails() {
        ...
        let err = adapter
            .execute(call_with_task("some task"))
    ...
    fn delegate_depth_resets_after_error() {
        ...
        let _ = adapter.execute(call_with_task("x"));
    ...
    fn delegate_depth_resets_after_validation_error() {
        ...
        let adapter = DelegateToolAdapter::with_agent(Box::new(NeverCalledAgent), 3, 0);
        let _ = adapter.execute(call_with_task(""));
    ...
    fn delegate_live_run() {
        let adapter = DelegateToolAdapter::new(3).expect("should build");
        let result = adapter.execute(call_with_task("echo hello"));
        assert!(result.is_ok());
    }
}
