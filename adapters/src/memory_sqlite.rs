...
impl SqliteMemoryAdapter {
    ...
    pub fn store_at(&mut self, key: &str, value: &str, updated_at: u64) -> AdapterResult<()> {
    ...
    pub fn prune_before(&mut self, cutoff_ms: u64) -> AdapterResult<usize> {
        let mut connection = self.connection.lock().unwrap();
        let transaction = connection
            .transaction_with_behavior(TransactionBehavior::Immediate)
            .map_err(map_sqlite_error)
...
impl crate::contracts::MemoryAdapter for SqliteMemoryAdapter {
    ...
    fn health(&self) -> crate::contracts::AdapterHealth {
        ...
        *cache = Some(HealthCache { result, checked_at: Instant::now() });
    ...
    fn get(&self, key: &str) -> AdapterResult<Option<crate::contracts::MemoryEntry>> {
        ...
        let connection = self.connection.lock().unwrap();
        let mut statement = connection.prepare(sql).map_err(map_sqlite_error).map_err(|e| map_memory_error("memory.sqlite.get", e))?;
        let mut rows = statement
            .query(rusqlite::params![key_hex.as_str()])
            .map_err(map_sqlite_error)
            .map_err(|e| map_memory_error("memory.sqlite.get", e))?;
        match rows.next().map_err(map_sqlite_error).map_err(|e| map_memory_error("memory.sqlite.get", e))? {
            Some(row) => Ok(Some(record_to_entry(decode_row(row).map_err(|e| map_memory_error("memory.sqlite.get", e))?))),
            None => Ok(None),
        }
    }

    fn list(&self) -> AdapterResult<Vec<crate::contracts::MemoryEntry>> {
        let records = self.fetch_records_no_params(&select_records_sql(0))
    ...
    fn delete(&mut self, key: &str) -> AdapterResult<bool> {
        ...
        transaction
            .execute("DELETE FROM memory_term_index WHERE key_hex = ?1;", rusqlite::params![key_hex.as_str()])
            .map_err(map_sqlite_error)
            .map_err(|e| map_memory_error("memory.sqlite.delete", e))?;
        let changed = transaction
            .execute("DELETE FROM memory WHERE key_hex = ?1;", rusqlite::params![key_hex.as_str()])
            .map_err(map_sqlite_error)
            .map_err(|e| map_memory_error("memory.sqlite.delete", e))?;
        transaction.commit().map_err(map_sqlite_error).map_err(|e| map_memory_error("memory.sqlite.delete", e))?;
        Ok(changed > 0)
    }
...
fn decode_row(row: &rusqlite::Row<'_>) -> MemoryResult<MemoryRecord> {
    ...
    let key = String::from_utf8(key_bytes).map_err(|error| {
        MemoryError::InvalidData(format!("invalid utf8 key in sqlite row `{key_hex}`: {error}"))
    })?;
