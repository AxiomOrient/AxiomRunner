use axiom_adapters::{AdapterHealth, ChannelAdapter, ChannelMessage, TelegramChannelAdapter, TelegramConfig};
...
fn channel_external_telegram_health_and_queue_semantics() {
    ...
    let first = channel
        .send(message("alerts", "first"))
        .expect("first send should succeed");
    let second = channel
        .send(message("alerts", "second"))
...
#[test]
fn channel_external_telegram_degrades_without_allowlist() {
    let config =
    ...
    assert_eq!(channel.health(), AdapterHealth::Degraded);
}
