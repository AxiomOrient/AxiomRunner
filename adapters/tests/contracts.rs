...
use axiom_adapters::{
    AdapterError, AdapterHealth, AdapterResult, ChannelAdapter, ChannelMessage, ChannelSendReceipt,
    MemoryAdapter, MemoryEntry, ProviderAdapter, ProviderRequest, ProviderResponse, RuntimeAdapter,
    RuntimeState, RuntimeTick, ToolAdapter, ToolCall, ToolOutput, build_contract_memory,
    build_contract_provider,
};
...
struct InMemoryStore {
    inner: HashMap<String, MemoryEntry>,
}
...
impl MemoryAdapter for InMemoryStore {
    ...
    fn list(&self) -> AdapterResult<Vec<MemoryEntry>> {
        let mut entries: Vec<MemoryEntry> = self.inner.values().cloned().collect();
        entries.sort_by(|a, b| a.key.cmp(&b.key));
        Ok(entries)
    }
...
fn contracts_provider_trait_object_smoke() {
    ...
    let response = provider
        .complete(ProviderRequest::new("test-model", "ping", 8))
        .expect("provider call should succeed");
    assert_eq!(response.content, "echo:ping");

    let error = provider
        .complete(ProviderRequest::new("test-model", " ", 8))
...
fn contracts_memory_trait_object_roundtrip_semantics() {
    let mut memory: Box<dyn MemoryAdapter> = Box::new(InMemoryStore::default());
...
fn contracts_channel_trait_object_queue_semantics() {
    ...
    let first = channel
        .send(ChannelMessage::new("t", "m1"))
        .expect("first send should succeed");
    let second = channel
        .send(ChannelMessage::new("t", "m2"))
        .expect("second send should succeed");
    assert_eq!(first.sequence, 1);
    assert_eq!(second.sequence, 2);

    let drained = channel.drain().expect("drain should succeed");
...
fn contracts_tool_trait_object_execution_semantics() {
    ...
    let ok = tool
        .execute(ToolCall::new("echo", args))
        .expect("known tool should execute");
    assert_eq!(ok.content, "ok:hello");

    let err = tool
        .execute(ToolCall::new("unknown", BTreeMap::new()))
...
fn contracts_real_provider_registry_implements_provider_contract() {
    ...
    let response = provider
        .complete(ProviderRequest::new("test-model", "ping", 16))
        .expect("provider call should succeed");
    assert_eq!(response.content, "ping");

    let error = provider
        .complete(ProviderRequest::new("test-model", "ping", 0))
        .expect_err("zero max_tokens should be rejected");
    assert_eq!(
        error,
