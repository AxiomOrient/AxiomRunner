...
#[cfg(test)]
mod tests {
    use super::{ENV_ALLOW_REMOTE, ENV_BIND, ENV_DEV_MODE, LOCALHOST_BIND, dev_mode_mitigation_active};

    #[test]
    fn dev_mode_mitigation_only_activates_for_explicit_local_tuple() {
        assert!(dev_mode_mitigation_active(Some(true), Some(true), Some(false)));
        assert!(!dev_mode_mitigation_active(Some(false), Some(true), Some(false)));
        assert!(!dev_mode_mitigation_active(Some(true), Some(false), Some(false)));
        assert!(!dev_mode_mitigation_active(Some(true), Some(true), Some(true)));
    }
