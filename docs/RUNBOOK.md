...
- `outcome`
- `execution_workspace`
- `resume`은 `waiting_approval` 상태의 goal-file pending run에서만 허용된다.
...
- mutating command(`run`, `resume`, `abort`)는 workspace별 `.axonrunner/runtime.lock`을 먼저 잡아야 한다.
...
- isolated worktree run이 `failed` 또는 `blocked`로 끝나면 `.axonrunner/artifacts/<intent>.rollback.json` 을 본다.
