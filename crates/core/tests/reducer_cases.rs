...
fn reducer_cases_policy_evaluated_allowed_updates_audit_without_denied_increment() {
    let before = AgentState {
        ...
        audit_count: 4,
        denied_count: 2,
        last_policy_code: Some(PolicyCode::ActorMissing),
        ..AgentState::default()
    };
    ...
    assert_eq!(after.last_policy_code, Some(PolicyCode::Allowed));
    assert_eq!(before.audit_count, 4);
    assert_eq!(before.denied_count, 2);
    assert_eq!(before.last_policy_code, Some(PolicyCode::ActorMissing));
}
...
fn reducer_cases_policy_evaluated_denied_increments_denied_counter() {
    ...
    assert_eq!(after.last_policy_code, Some(PolicyCode::ReadOnlyMutation));
}
...
fn reducer_cases_policy_counters_and_revision_use_saturating_math() {
    ...
    assert_eq!(after.last_policy_code, Some(PolicyCode::RuntimeHalted));
}
