...
impl WorkspaceTool {
    ...
    fn execute_shell(&self, command: &str) -> Result<ToolResult, ToolError> {
        ...
        let binary = parts
            .next()
            .ok_or(ToolError::InvalidInput("shell command"))?;
...
pub fn resolve_tool_adapter_id(name: &str) -> Option<&'static str> {
    resolve_tool_entry(name).map(|entry| entry.adapter_id)
}
