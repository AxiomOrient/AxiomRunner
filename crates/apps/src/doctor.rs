...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DoctorRuntimeInput<'a> {
...
pub struct DoctorInput<'a> {
    pub context: DoctorContextInput<'a>,
    pub runtime: DoctorRuntimeInput<'a>,
...
pub fn build_doctor_report(input: DoctorInput<'_>) -> DoctorReport {
    ...
    let endpoint_level = if has_http_scheme(input.context.endpoint) {
    ...
    checks.push(DoctorCheck {
        ...
        detail: format!("endpoint={}", input.context.endpoint),
    });

    let mode_level = if input.runtime.mode == ExecutionMode::Halted {
        ok = false;
        DoctorCheckLevel::Warn
    } else {
        DoctorCheckLevel::Pass
    ...
    checks.push(DoctorCheck {
        name: "runtime_mode",
        level: mode_level,
        detail: format!("mode={}", mode_name(input.runtime.mode)),
    });

    let provider_level = if input.runtime.provider_model.trim().is_empty() {
        ok = false;
        DoctorCheckLevel::Warn
    } else {
        DoctorCheckLevel::Pass
    ...
    checks.push(DoctorCheck {
        ...
        detail: format!("provider_model={}", input.runtime.provider_model),
    });

    checks.push(DoctorCheck {
        name: "memory_adapter",
        level: DoctorCheckLevel::Info,
        detail: format!("enabled={}", input.runtime.memory_enabled),
    });
    checks.push(DoctorCheck {
        name: "tool_adapter",
        level: DoctorCheckLevel::Info,
        detail: format!("enabled={}", input.runtime.tool_enabled),
    });
    ...
    DoctorReport {
        ok,
        profile: input.context.profile.to_owned(),
        endpoint: input.context.endpoint.to_owned(),
