...
fn parse_onboard_command(args: &[String]) -> Result<CliCommand, String> {
    ...
    while index < args.len() {
        ...
        match arg.as_str() {
            ...
            "--api-key" => api_key = Some(next_arg(args, &mut index, "onboard", "--api-key")?.to_owned()),
            "--provider" => provider = Some(next_arg(args, &mut index, "onboard", "--provider")?.to_owned()),
            "--memory" => memory = Some(next_arg(args, &mut index, "onboard", "--memory")?.to_owned()),
            _ => {
...
fn parse_agent_command(args: &[String]) -> Result<CliCommand, String> {
    ...
    while index < args.len() {
        ...
        match arg.as_str() {
            ...
            "--message" | "-m" => message = Some(next_arg(args, &mut index, "agent", arg)?.to_owned()),
    ...
    Ok(CliCommand::Agent {
        action: AgentActionTemplate { cwd, message, model },
    })
}
...
fn parse_migrate_command(args: &[String]) -> Result<CliCommand, String> {
...
fn parse_channel_serve_args(args: &[String]) -> Result<u64, String> {
    ...
    while index < args.len() {
        ...
        if arg == "--poll-interval" {
            index += 1;
            if index >= args.len() {
                return Err(format!("channel serve --poll-interval requires a value\n{USAGE}"));
            }
            poll_interval_secs = args[index]
                .trim()
                .parse::<u64>()
                .map_err(|_| format!("channel serve --poll-interval must be a non-negative integer\n{USAGE}"))?;
...
fn parse_integrations_command(args: &[String]) -> Result<CliCommand, String> {
    let action = args
        .first()
        .ok_or_else(|| format!("command 'integrations' requires 'info|install|remove|list'\n{USAGE}"))?;
    let rest = &args[1..];
...
fn next_arg<'a>(args: &'a [String], index: &mut usize, cmd: &str, flag: &str) -> Result<&'a str, String> {
    *index += 1;
    if *index >= args.len() {
        return Err(format!("command '{cmd}' requires value for {flag}\n{USAGE}"));
    }
...
pub(crate) fn parse_intent_spec_raw(spec: &str) -> Result<IntentSpecVariant, String> {
    ...
    if let Some(key) = trimmed.strip_prefix("read:") {
        ...
        return Ok(IntentSpecVariant::Read { key: key.to_owned() });
    }
    if let Some(key) = trimmed.strip_prefix("remove:") {
        ...
        return Ok(IntentSpecVariant::Remove { key: key.to_owned() });
    }
    if let Some(payload) = trimmed.strip_prefix("write:") {
        ...
        if value.is_empty() {
            return Err(format!("invalid intent-spec '{spec}': write value is empty"));
        }
        return Ok(IntentSpecVariant::Write { key: key.to_owned(), value: value.to_owned() });
    }
