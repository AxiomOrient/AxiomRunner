...
pub fn execute_agent_action(
    ...
    let AgentExecutionContext { agent, estop, context } = ctx;
...
fn parse_interactive_script_from_env() -> Vec<String> {
    ...
    if from_env.is_empty() {
        DEFAULT_INTERACTIVE_SCRIPT.iter().map(|s| s.to_string()).collect()
    } else {
        from_env
    }
}
...
fn enrich_with_context(message: &str, hits: &[axiom_adapters::contracts::ContextHit]) -> String {
    ...
    let capacity = "[RETRIEVED CONTEXT]\n".len()
        + hits.iter().map(|h| h.uri.len() + h.snippet.len() + 32).sum::<usize>()
        + "\n[USER MESSAGE]\n".len()
        + message.len();
    let mut enriched = String::with_capacity(capacity);
    enriched.push_str("[RETRIEVED CONTEXT]\n");
    for (i, hit) in hits.iter().enumerate() {
        writeln!(enriched, "{}. [score={:.3}] {}: {}", i + 1, hit.score, hit.uri, hit.snippet).ok();
    }
    enriched.push_str("\n[USER MESSAGE]\n");
...
mod tests {
    use super::{
        AgentAction, AgentExecutionContext, AgentResultKind, AgentStopReason,
        execute_agent_action, execute_interactive, is_stop_command,
    };
